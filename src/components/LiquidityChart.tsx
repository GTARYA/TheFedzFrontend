...
const LiquidityChart = ({
    ...
    useEffect(() => {
        ...
        const x = d3.scaleLinear().domain([minTick, maxTick]).range([0, width]);
        ...
        const drawTickSelector = (initialTick, color, type) => {
            ...
            const drag = d3
                ...
                .on('drag', (event) => {
                    ...
                    const mouseX =
                        event.sourceEvent.clientX - svgRect.left - margin.left;
                ...
                .on('end', (event) => {
                    ...
                    let newTick =
                        Math.round(x.invert(mouseX) / tickSpacing) *
                        tickSpacing;
                    // Ensure newTick is within the domain of x scale
                    newTick = Math.max(
                        x.domain()[0],
                        Math.min(x.domain()[1], newTick)
                    );
    ...
    return (
        <div ref={containerRef} className="w-full card">
            <svg
                ...
                className="card-body flex flex-col"></svg>
        </div>
    );
};
